...
void spi_main_lcd(void)
{
   ...
   // Initialize the SPI bus
    ...
    ESP_LOGI(TAG, "Initializing external SPI");
    ESP_LOGI(TAG, "Pin assignments:");
    ESP_LOGI(TAG, "MOSI: %2d   MISO: %2d   SCLK: %2d   CS: %2d",
        bus_config.mosi_io_num, bus_config.miso_io_num,
        bus_config.sclk_io_num, devcfg.spics_io_num
    );

    while (1)
    {
        vTaskDelay(1000 / portTICK_PERIOD_MS);
        ESP_LOGI(TAG, "Request ID -> %d :",get_id(spi));
...
void app_main(void)
{
    /* Configure the IOMUX register for pad BLINK_GPIO (some pads are
       ...
       functions.)
    */
    xTaskCreate(spi_main_lcd,"spi_main_task",10000,NULL,tskIDLE_PRIORITY+1,NULL);
